...
import { LABEL_TABLE_COLUMNS } from '/@/renderer/components/virtual-table';
...
const FILTERS = {
    jellyfin: [
        ...
        },
        {
            ...
            name: i18n.t('entity.albumCount', { postProcess: 'titleCase' }),
            value: 'albumCount',
        },
    ],
    navidrome: [
...
type LabelListFilter = LabelListQuery & {
    sortOrder: SortOrder;
};

interface LabelListHeaderFiltersProps {
    gridRef: MutableRefObject<null | VirtualInfiniteGridRef>;
    itemCount: number | undefined;
    tableRef: MutableRefObject<AgGridReactType | null>;
}

export const LabelListHeaderFilters = ({
    gridRef,
    itemCount,
    tableRef,
}: LabelListHeaderFiltersProps) => {
    ...
    const { display, filter, grid, table } = useListStoreByKey<LabelListQuery>({ key: pageKey });
    ...
    const onFilterChange = useCallback(
        (filterData: LabelListFilter) => {
            if (isGrid) {
                handleRefreshGrid(gridRef, Object.assign({}, filterData, customFilters));
            } else {
                handleRefreshTable(tableRef, Object.assign({}, filterData, customFilters));
            }
        },
    ...
    const handleRefresh = useCallback(() => {
        queryClient.invalidateQueries(queryKeys.labels.list(server?.id || ''));
        const filterData = filter ? { ...filter } : {};
        onFilterChange(filterData as LabelListFilter);
    ...
    const handleToggleSortOrder = useCallback(() => {
        const currentSortOrder = filter?.sortOrder;
